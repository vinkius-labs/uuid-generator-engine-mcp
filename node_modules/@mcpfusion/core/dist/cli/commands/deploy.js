...
const BUILTIN_FILTER = new RegExp(`^(node:)?(${BUILTIN_ROOTS.join('|')})(/.*)?$`);
// ── Bundle Sanitizer ─────────────────────────────────────────────────────────
...
function sanitizeBundleForEdge(code) {
    return code
...
export async function commandDeploy(args) {
    ...
    const _magenta = (s) => `\x1b[35m${s}\x1b[0m`;
    const _bgGreen = (s) => `\x1b[42m\x1b[30m${s}\x1b[0m`;
