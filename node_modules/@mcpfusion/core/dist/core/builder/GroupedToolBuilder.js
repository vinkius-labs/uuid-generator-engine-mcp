 ...
 * @example
 * ```typescript
 * import { createTool, success, error } from '@mcpfusion/core';
 * import { z } from 'zod';
 *
...
import { mergeHooks } from '../execution/PipelineHooks.js';
import { buildDebugHooks, buildTracedHooks, buildTelemetryHooks } from './ObservabilityHooks.js';
...
export class GroupedToolBuilder {
     ...
     * @example
     * ```typescript
     * import { createTool, createDebugObserver, success } from '@mcpfusion/core';
     *
     ...
     *
     * **OTel direct pass-through**: The `MCPFusionTracer` interface is a
     ...
     * @returns `this` for chaining
     *
     * @see {@link MCPFusionTracer} for the interface contract
    ...
    mergeActions(actions) {
        ...
        for (const action of actions) {
            if (this._actions.some(a => a.key === action.key)) {
                throw new Error(`Duplicate action "${action.key}" on builder "${this._name}" during merge.`);
            }
