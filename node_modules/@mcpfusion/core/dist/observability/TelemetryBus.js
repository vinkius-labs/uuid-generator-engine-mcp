 ...
 */
export function getTelemetryPath(fingerprint) {
 ...
 * @internal
 */
function writeRegistryFile(pid, serverName) {
    try {
...
export async function createTelemetryBus(config) {
    ...
    const chmodSocket = () => {
        if (platform() !== 'win32') {
            try {
                ...
                process.stderr.write('[mcpfusion] Warning: Could not restrict socket permissions.\n');
            }
        }
    };
    ...
    const sigintHandler = () => sigHandler('SIGINT');
    const sigtermHandler = () => sigHandler('SIGTERM');
