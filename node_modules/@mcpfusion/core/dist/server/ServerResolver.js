 ...
 */
function isMcpServerLike(obj) {
    return (typeof obj === 'object' &&
        obj !== null &&
        typeof obj['setRequestHandler'] === 'function');
}
function hasServerProperty(obj) {
    return typeof obj === 'object' && obj !== null && 'server' in obj;
}
// ── Resolver Chain ───────────────────────────────────────
/** Resolve direct Server instance */
const directServerResolver = (input) => isMcpServerLike(input) ? input : undefined;
/** Resolve McpServer wrapper (has .server property exposing low-level Server) */
const wrappedServerResolver = (input) => hasServerProperty(input) && isMcpServerLike(input.server) ? input.server : undefined;
...
export function resolveServer(server) {
    ...
    const receivedKeys = Object.keys(server).slice(0, 10).join(', ') || '(none)';
    throw new Error('attachToServer() requires a Server or McpServer instance. ' +
