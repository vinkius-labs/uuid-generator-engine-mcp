...
function parseArrayHeaderLine(content, defaultDelimiter) {
	...
	if (braceStart !== -1 && braceStart < content.indexOf(":", bracketEnd)) {
		if (content.slice(bracketEnd + 1, braceStart).trim() !== "") return;
	...
	if (content.slice(gapStart, colonIndex).trim() !== "") return;
	let key;
	if (bracketStart > 0) {
...
function* decodeKeyValueSync(line, cursor, baseDepth, options) {
...
async function* decodeKeyValueAsync(line, cursor, baseDepth, options) {
	const content = line.content;
	const arrayHeader = withLine(line, () => parseArrayHeaderLine(content, DEFAULT_DELIMITER));
	if (arrayHeader && arrayHeader.header.key !== void 0) {
		yield {
